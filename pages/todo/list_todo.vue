		...
		<view class="filter-overlay" v-if="isFilterOpen" @click.stop="closeFilter">
			<view class="filter-panel" @click.stop>
				...
				<scroll-view scroll-y="true" class="filter-body">
					<view class="f-group"><text class="f-label">Tiêu đề / Từ khóa</text><input class="f-input" v-model="filter.title" placeholder="Nhập từ khóa..." /></view>
					<view class="f-group"><text class="f-label">Mã công việc</text><input class="f-input" v-model="filter.jobCode" placeholder="Ví dụ: TODO-08" /></view>
					<view class="f-group"><text class="f-label">Trạng thái</text><picker mode="selector" :range="statusOptions" :value="statusIndex" @change="onStatusChange"><view class="f-picker">{{ statusOptions[statusIndex] }}<text class="arrow">▼</text></view></picker></view>
					<view class="f-section-title">Thời gian tạo</view>
					<view class="f-row">
						<view class="f-group half"><picker mode="date" :value="filter.createdFrom" @change="(e) => filter.createdFrom = e.detail.value"><view class="f-picker date">{{ filter.createdFrom || 'Từ ngày' }}</view></picker></view>
						<view class="f-group half"><picker mode="date" :value="filter.createdTo" @change="(e) => filter.createdTo = e.detail.value"><view class="f-picker date">{{ filter.createdTo || 'Đến ngày' }}</view></picker></view>
					</view>
				</scroll-view>
...
<script setup>
	...
	const { 
		todos, isLoading, isFilterOpen, 
		statusOptions, statusIndex, filter, 
		isConfirmDeleteOpen, itemToDelete,
		// --- New Props ---
		pageSizeOptions, pageSizeIndex, currentPage, onPageSizeChange,
		// ----------------
		addNewTask, openFilter, closeFilter, 
		onStatusChange, resetFilter, applyFilter,
...
<style lang="scss" scoped>
	...
	.fixed-footer { 
		...
		/* Flex Layout cho 3 thành phần */
		display: flex; 
		justify-content: space-between; 
		align-items: center;
	}

	/* 1. Style Dropdown Số lượng/trang */
	.page-size-selector {
		display: flex;
		align-items: center;
	...
	.add-text {
		font-size: 14px; /* Giảm xíu size chữ để vừa màn hình nhỏ */
		font-weight: 600;
		white-space: nowrap;
	}
	/* ---------------- */
	...
	.bg-green { background-color: #2dce89; } .bg-blue { background-color: #11cdef; } .bg-orange { background-color: #fb6340; }
	...
	/* Filter Popup & Modal Styles (Giữ nguyên) */
